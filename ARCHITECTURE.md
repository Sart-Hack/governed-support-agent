...
**Follow-up #1 RESOLVED (P2-M6, 2026-06-04).** The agent now calls Bifrost as its LLM endpoint. `OPENAI_API_KEY` is populated in `.env`; Bifrost auto-registers the `openai` provider from its container env on boot (no manual provider POST needed), confirmed with a `POST /v1/chat/completions` round-trip to `openai/gpt-4o-mini`. The `support-ops` Mastra workflow's triage step issues a chat completion with `tools` + `tool_choice: auto`, and the model emits a `replyInternal` tool call that the agent gates (Cedar + scope-check) and executes via `@gsa/mcp-client`. Architecture note: the governed execution path runs tool calls through `@gsa/mcp-client` (so every call passes the scope-check and Cedar), NOT through Bifrost's `/v1/mcp/tool/execute` code-mode endpoint. Bifrost is the LLM gateway (reasoning + token accounting); agent-shield is the policy-decision point. `pnpm demo` plays scenario 1 (TCK-1) end-to-end: classify → KB search → plan (tool_choice:auto) → policy-check (allow, policy 01/ASI02) → no approval → execute internal note → audit, with the full `policy.decision → scope.granted → tool.call → tool.result` chain per tool in the audit log.
