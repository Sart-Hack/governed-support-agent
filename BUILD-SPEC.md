...
`TODO: confirm with user` — Bifrost vs Portkey (post-PANW). Recommendation: **Bifrost**.
...
`TODO: confirm with user` — real-API MCP server choice: **GitHub Issues (free-tier OAuth)** vs Linear. Recommendation: **GitHub** (buyer pattern recognition: Tailscale/PostHog/Alpaca all live in GitHub). Linear is the in-domain alternative.
...
`TODO: confirm with user` — scenario set above. Recommendation: **the seven listed**.
...
`TODO: confirm with user` — AgentDojo. Recommendation: **drop**.
...
`TODO: confirm with user` — accept 6-9 week calendar OR cut to 80h via hour-60 MVD path. Recommendation: **accept the 6-9 week calendar**; ship the full demo, not a thin one.
...
2. GitHub real-API MCP (revert to a mock; defer real to v2)
3. Two scenarios — drop kill-switch, then PII-redaction (keep refusals)
4. `/tenants` page (move policy 07 to a code block on `/policies`)
5. Cost overlay on `/traces` (replace with static screenshot)
6. Excalidraw on `/architecture` (keep SVG)
7. Permission matrix interactive hover (static table)
8. Monaco PolicyEditor (syntax-highlighted code blocks)
9. Loom video (record week 4)
10. Auto-updating eval badge (manually commit `badge.json`)
...
1. **Functional:** `pnpm demo` on a clean machine plays all 7 scenarios end-to-end. Postgres checkpoint survives a `docker compose restart` between approval-gate and execute. Slack approval round-trips. Cedar denies the refusal cases via `agent-shield`. Circuit breaker trips on the $0.50 ceiling (scenario 2). Kill switch halts an in-flight run within 1s (scenario 7). GitHub Issue gets filed against the demo org (scenario 1).
...
Each carries a recommendation. The PR opening this spec asks the user to confirm or override.

1. **LLM gateway:** Bifrost (Apache 2.0, native MCP, by Maxim/evals) vs Portkey (post-PANW). `TODO: confirm with user` — Recommendation: **Bifrost**.
2. **Calendar:** accept realistic 6-9 weeks at 20-25h/wk vs ruthless cut to 80h/hour-60-MVD. `TODO: confirm with user` — Recommendation: **accept the longer calendar**.
3. **Scenario set:** the 7 listed above (drop spam + priv-esc; add cost-ceiling + kill-switch; reframe injection as indirect-from-Notion). `TODO: confirm with user` — Recommendation: **the seven listed**.
4. **Real-API MCP server:** GitHub Issues (free-tier OAuth) vs Linear (in-domain). `TODO: confirm with user` — Recommendation: **GitHub** (buyer pattern recognition).
5. **Mastra workflow API:** v2 vs legacy. `TODO: confirm with user` — Recommendation: **v2**.
6. **AgentDojo:** keep or drop. `TODO: confirm with user` — Recommendation: **drop** (API instability).
